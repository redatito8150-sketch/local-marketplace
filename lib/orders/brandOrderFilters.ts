...
export function filterBrandOrders(orders: BrandOrder[], filters: BrandOrderFilters) {
  ...
  const filtered = orders.filter((order) => {
    ...
    if (query && !`${order.orderNumber} ${order.shippingName} ${order.shippingCity} ${order.items.map((item) => `${item.name} ${item.color ?? ""} ${item.size}`).join(" ")}`.toLocaleLowerCase().includes(query)) return false;
