...
type StockParams = { brand?: string; q?: string; level?: string; sort?: string; product?: string; view?: string };
      ...
      {view === "inventory" && <>
        ...
        <form action="/brand-portal/stock" className="mt-4 rounded-[18px] border border-[#eadfd7] bg-white p-4">
          ...
          <div className="grid gap-3 lg:grid-cols-[minmax(260px,1fr)_190px_auto] lg:items-end">
            ...
            <div className="flex gap-2"><button type="submit" className="h-11 rounded-xl bg-[#C85956] px-5 text-[12px] font-bold text-white transition-colors hover:bg-[#b84e4b] focus-visible:outline-none focus-visible:ring-4 focus-visible:ring-[#C85956]/20">Apply</button>{activeFilterCount > 0 && <Link href={href({ q: undefined, level: undefined, sort: undefined, product: undefined, view: undefined })} className="inline-flex h-11 items-center px-2 text-[11px] font-bold text-[#8d8076] hover:text-[#C85956]">Clear</Link>}</div>
          </div>
        </form>
      </>}

      <div className="mt-4">
        {view === "inventory" && !variants.length ? <DashboardEmptyState title="No matching inventory" description={activeFilterCount ? "Try another stock level or clear the current search." : "Product variants will appear here after catalog setup."} /> : <InventoryManager variants={variants} allVariants={allVariants} history={history} brandSlug={owner.brandSlug} isMahalyPartner={owner.isMahalyPartner} readOnly={owner.isImpersonating} view={view} />}
      </div>
    </div>
  );
}
