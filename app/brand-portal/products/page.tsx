...
import ProductArchiveButton from "@/components/brand-portal/ProductArchiveButton";
...
export default async function BrandPortalProductsPage(props: { searchParams: Promise<ProductParams> }) {
  ...
  return (
    <div>
      ...
      <DashboardPageHeader eyebrow="Catalog" title={`Products (${products.length})`} description={`${allProducts.length} products in your catalog. Product changes continue to use the existing Zakhnook review workflow.`} actions={<Link href={`/brand-portal/products/new${brandParam}`} className={dashboardButtonPrimary}><Plus className="mr-2 h-4 w-4" />Add product</Link>} />
      ...
      <DashboardPanel className="mt-6">
        {products.length ? <div className="overflow-x-auto"><table className="w-full min-w-[850px] text-left text-[13px]"><thead className="border-b border-[#e8e0d7] bg-[#fbf8f4] text-[10.5px] uppercase tracking-[0.08em] text-[#897b70]"><tr><th className="px-5 py-3 font-semibold">Product</th><th className="px-5 py-3 font-semibold">Category</th><th className="px-5 py-3 font-semibold">Price</th><th className="px-5 py-3 font-semibold">Inventory</th><th className="px-5 py-3 font-semibold">Status</th><th className="px-5 py-3" /></tr></thead><tbody className="divide-y divide-[#eee7de]">{products.map((product) => {
          ...
          // A "published" row with a future Publish Date isn't actually
          // live yet (see lib/newArrivals.ts's isPublishDateLive, the same
          // gate the storefront queries use) — showing it as plain
          // "Published" here would mislead the brand owner into thinking
          // customers can already see it.
          ...
          const statusInfo = daysLeft != null
            ...
            : isScheduled
            ? { label: `Scheduled — ${new Date(product.publishDate!).toLocaleString("en-GB", { day: "numeric", month: "short", hour: "2-digit", minute: "2-digit" })}`, className: "bg-blue-50 text-blue-700" }
            : baseStatusInfo;
          const editHref = `/brand-portal/products/${product.id}/edit${brandParam}`;
          return <tr key={product.id} className="hover:bg-[#fbf8f4]"><td className="px-5 py-4"><div className="flex items-center gap-3"><div className="relative h-11 w-11 flex-none overflow-hidden rounded-xl bg-[#f1eae2]"><Image src={product.image} alt={product.name} fill className="object-cover" /></div><div><p className="font-bold text-[#242424]">{product.name}</p>{product.collection && <p className="mt-0.5 text-[11px] text-[#8a7d73]">{product.collection}</p>}</div></div></td><td className="px-5 py-4 text-[#75685f]"><p>{product.mainCategory ?? "—"}</p>{product.productType && <p className="mt-0.5 text-[11px] text-[#9b8e84]">{product.productType}</p>}</td><td className="px-5 py-4 font-bold text-[#242424]">{formatPrice(product.price, product.currency)}</td><td className="px-5 py-4"><span className={`rounded-full px-2.5 py-1 text-[10.5px] font-bold ${product.inStock ? "bg-emerald-50 text-emerald-700" : "bg-red-50 text-red-700"}`}>{product.inStock ? "In stock" : "Out of stock"}</span></td><td className="px-5 py-4"><div className="flex flex-wrap gap-1.5"><span className={`rounded-full px-2.5 py-1 text-[10.5px] font-bold ${statusInfo.className}`}>{statusInfo.label}</span>{product.hasPendingEdit && <span className="rounded-full bg-amber-50 px-2.5 py-1 text-[10.5px] font-bold text-amber-700">Edit pending</span>}{product.pausedByBrand && <span className="rounded-full bg-slate-100 px-2.5 py-1 text-[10.5px] font-bold text-slate-600">Paused</span>}</div>{product.reviewNotes && <p className="mt-1.5 max-w-xs text-[11px] text-red-600">{product.reviewNotes}</p>}</td><td className="px-5 py-4"><div className="flex items-center justify-end gap-1">{product.status === "published" && <ProductArchiveButton productId={product.id} name={product.name} />}<Link href={editHref} aria-label={`Edit ${product.name}`} className="rounded-lg p-2 text-[#8a7d73] hover:bg-[#f1eae2] hover:text-[#242424]"><Pencil className="h-4 w-4" /></Link><RequestDeletionButton productId={product.id} name={product.name} alreadyRequested={Boolean(product.deletionRequestedAt)} /></div></td></tr>;
