...
export async function PATCH(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (body.action === "delete") {
    ...
    if (error) return safeErrorResponse("brand-portal.collections.delete", error);
    await logAudit({ actorId: owner.user.id, actorLabel, entityType: "collection", entityId: id, action: "delete", before: collection, brandSlug: owner.brandSlug ?? undefined });
  ...
  if (error) return safeErrorResponse("brand-portal.collections.update", error);
  await logAudit({
  ...
  return NextResponse.json({ updated: true, references });
}
