...
export async function POST(request:NextRequest,{params}:{params:Promise<{id:string}>}){
  ...
  const user=await getRequestUser(request);if(!user)return NextResponse.json({error:"Sign in required"},{status:401});
  ...
  if(!review||review.user_id===user.id)return NextResponse.json({error:"This review cannot be reported."},{status:403});
  const {error}=await supabaseAdmin.from("review_reports").insert({review_id:id,reporter_user_id:user.id,...parsed.data});
