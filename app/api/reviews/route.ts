...
export async function POST(request: NextRequest) {
  ...
  const {data:review,error}=await supabase.from("reviews").insert({...parsed.data,user_id:user.id}).select("id").single();
  if(error)return NextResponse.json({error:error.code==="23505"?"This purchase has already been reviewed.":"You are not eligible to review this purchase."},{status:403});
