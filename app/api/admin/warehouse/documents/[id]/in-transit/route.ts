...
export async function POST(request: NextRequest, props: { params: Promise<{ id: string }> }) {
  ...
  if (transfer.direction === "to_brand") {
    ...
    if (brand?.owner_user_id) await notifyUser(
      ...
      "The return has left Zakhnook and is now in transit. Open the Stock Return Note, complete every received, damaged and missing Document line, then confirm delivery. Your brand stock updates only after that confirmation.",
