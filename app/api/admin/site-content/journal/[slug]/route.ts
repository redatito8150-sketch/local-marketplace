...
export async function PATCH(request: NextRequest, props: { params: Promise<{ slug: string }> }) {
  ...
  if (error) {
    return safeErrorResponse("admin.site-content.journal.save", error, "Failed to save");
  }
...
export async function DELETE(_request: NextRequest, props: { params: Promise<{ slug: string }> }) {
  ...
  if (error) {
    return safeErrorResponse("admin.site-content.journal.delete", error, "Failed to delete");
  }
