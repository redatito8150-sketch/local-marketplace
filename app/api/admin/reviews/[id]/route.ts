...
export async function PATCH(request:NextRequest,{params}:{params:Promise<{id:string}>}){
 ...
 const {error}=await supabaseAdmin.from("reviews").update({status:parsed.data.status,moderation_reason:parsed.data.reason??null}).eq("id",id);if(error)return safeErrorResponse("admin.reviews.moderate",error);
