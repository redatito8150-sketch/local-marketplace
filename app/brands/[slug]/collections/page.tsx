    ...
    {collections.length > 0 && (
      <div className="mt-10 grid gap-5 sm:grid-cols-2 lg:grid-cols-3">
        {collections.map((collection) => (
          <Link
            key={collection.id}
            href={`/brands/${slug}/collections/${collection.slug}`}
            className="group overflow-hidden rounded-[22px] border border-[#e6ddd2] bg-[#fbf7f1]"
          >
            <div className="relative aspect-[4/3] bg-[#efe6dc]">
              {collection.coverImageUrl && (
                <Image src={collection.coverImageUrl} alt={collection.name} fill sizes="(max-width: 768px) 100vw, 33vw" className="object-cover transition duration-500 group-hover:scale-[1.03]" />
              )}
            </div>
            <div className="p-5">
              <h3 className="font-serif text-xl text-[#261f1b]">{collection.name}</h3>
              {collection.description && (
                <p className="mt-1.5 line-clamp-2 text-[13px] leading-5 text-[#736861]">{collection.description}</p>
              )}
            </div>
          </Link>
        ))}
