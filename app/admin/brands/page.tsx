    ...
    <div>
      ...
      <DashboardPanel className="mt-6">
        {brands.length ? <div className="overflow-x-auto"><table className="w-full min-w-[900px] text-left text-[13px]"><thead className="border-b border-slate-200 bg-slate-50/80 text-[10.5px] uppercase tracking-[0.08em] text-slate-500"><tr><th className="px-5 py-3 font-semibold">Brand</th><th className="px-5 py-3 font-semibold">Category</th><th className="px-5 py-3 font-semibold">City</th><th className="px-5 py-3 font-semibold">Owner account</th><th className="px-5 py-3 font-semibold">Status</th><th className="px-5 py-3" /></tr></thead><tbody className="divide-y divide-slate-100">{brands.map((brand) => (
          <tr key={brand.slug} className="hover:bg-slate-50/70"><td className="px-5 py-4"><div className="flex items-center gap-3"><div className="relative h-11 w-11 overflow-hidden rounded-xl bg-slate-100"><Image src={brand.heroImage} alt={brand.name} fill className="object-cover" /></div><div><p className="font-bold text-slate-900">{brand.name}</p><p className="mt-0.5 text-[11px] text-slate-500">/{brand.slug}</p></div></div></td><td className="px-5 py-4 text-slate-600">{brand.category}</td><td className="px-5 py-4 text-slate-600">{brand.city}</td><td className="px-5 py-4">{brand.ownerEmail ? <div><span className="rounded-full bg-emerald-50 px-2.5 py-1 text-[10.5px] font-bold text-emerald-700">Linked</span><p className="mt-1.5 text-[10.5px] text-slate-500">{brand.ownerEmail}</p></div> : <span className="rounded-full bg-amber-50 px-2.5 py-1 text-[10.5px] font-bold text-amber-700">Not linked</span>}</td><td className="px-5 py-4"><div className="flex flex-wrap items-center gap-1.5"><BrandQuickToggle slug={brand.slug} field="isActive" active={brand.isActive} label="Active" /><BrandQuickToggle slug={brand.slug} field="isMahalyPartner" active={brand.isMahalyPartner} label="Partner" /><BrandQuickToggle slug={brand.slug} field="isSponsored" active={brand.isSponsored} label="Sponsored" />{brand.isSponsored && <Link href={`/admin/brands/${brand.slug}/edit`} className="text-[10.5px] font-semibold text-mahalyred hover:underline">Configure placements →</Link>}</div></td><td className="px-5 py-4"><div className="flex items-center justify-end gap-1"><Link href={`/brand-portal?brand=${brand.slug}`} aria-label={`View ${brand.name}'s portal`} className="rounded-lg p-2 text-slate-500 hover:bg-slate-100 hover:text-slate-900"><LayoutDashboard className="h-4 w-4" /></Link><Link href={`/admin/brands/${brand.slug}/edit`} aria-label={`Edit ${brand.name}`} className="rounded-lg p-2 text-slate-500 hover:bg-slate-100 hover:text-slate-900"><Pencil className="h-4 w-4" /></Link><DeleteEntityButton apiPath={`/api/admin/brands/${brand.slug}`} name={brand.name} /></div></td></tr>
