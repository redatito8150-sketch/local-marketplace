    ...
    <div>
      ...
      <DashboardPanel className="mt-6">
        {logs.length ? <div className="divide-y divide-slate-100">{logs.map((log) => {
          const changes = diffEntitySnapshots(log.beforeValue, log.afterValue);
          const entityPath = getEntityAdminPath(log.entityType, log.entityId);
          return (
          <details key={log.id} className="group px-5 py-4 open:bg-slate-50/60"><summary className="flex cursor-pointer list-none flex-col gap-3 sm:flex-row sm:items-center sm:justify-between"><div className="flex min-w-0 items-start gap-3"><span className="mt-0.5 rounded-full bg-slate-100 px-2.5 py-1 text-[10px] font-bold text-slate-600">{ENTITY_LABELS[log.entityType] ?? log.entityType}</span><div className="min-w-0"><p className="text-[12.5px] leading-5 text-slate-800">{describeAuditLog(log)}</p><p className="mt-1 text-[10.5px] font-medium uppercase tracking-[0.06em] text-slate-400">{ACTION_LABELS[log.action] ?? log.action}</p></div></div><time className="whitespace-nowrap pl-0 text-[11px] text-slate-400 sm:pl-4">{formatDateTime(log.createdAt)}</time></summary>
            <div className="mt-4 space-y-3 text-[11.5px]">
              <div>
                <p className="mb-1.5 text-[10px] font-bold uppercase tracking-[0.08em] text-slate-400">What changed</p>
                {changes.length ? (
                    ...
                    {changes.map((change) => (
                      <li key={change.field} className="text-slate-600">
                        ...
                        {change.from !== undefined && change.to !== undefined
                          ? <>{change.from} <span className="text-slate-400">→</span> {change.to}</>
                          : (change.to ?? change.from)}
                      </li>
                    ))}
                  </ul>
                ) : (
                  <p className="rounded-xl border border-dashed border-slate-200 bg-white p-3 text-slate-400">No field-level changes recorded.</p>
                )}
              </div>
              {entityPath && (
                <Link href={entityPath} className="inline-block text-[11.5px] font-semibold text-mahalyred hover:underline">
                  Open in admin →
                </Link>
              )}
            </div>
