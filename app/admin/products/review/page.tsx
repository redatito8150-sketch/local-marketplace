...
type ActivityParams = { q?: string; type?: string; from?: string; to?: string };
...
export default async function BrandActivityPage(props: { searchParams: Promise<ActivityParams> }) {
  ...
  const activity = allActivity.filter((item) => {
    ...
    if (params.to && new Date(item.createdAt) > new Date(`${params.to}T23:59:59.999`)) return false;
    return true;
  });
  const types = [...new Set(allActivity.map((item) => item.type))].sort();
  const activeCount = [params.q, params.type, params.from, params.to].filter(Boolean).length;
  ...
  return (
    <div>
      ...
      <DashboardFilters action="/admin/products/review" clearHref="/admin/products/review" activeCount={activeCount}>
        ...
        <DashboardMoreFilters label="More activity filters" active={Boolean(params.type)}>
          <DashboardFilterField label="Activity type"><select name="type" defaultValue={params.type ?? ""} className={`${dashboardFilterControl} w-full`}><option value="">All types</option>{types.map((value) => <option key={value} value={value}>{NOTIFICATION_TYPE_LABELS[value] ?? value}</option>)}</select></DashboardFilterField>
