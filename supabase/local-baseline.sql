...
--
-- This is the schema snapshot immediately before the repository's first
-- timestamped migration (parent of commit 686f1a6). The local Supabase
-- launcher copies it ahead of supabase/migrations so a blank local database
-- can replay the same history production received. It is intentionally not
-- stored in supabase/migrations and must never be pushed to a remote project.
...
-- ============================================================================
-- LIVE-ONLY CUSTOMER TABLES
--
-- These four tables predate the tracked migration history. Production has
-- always depended on them, but their CREATE TABLE statements were never
-- committed. They belong in this local-only historical baseline so a fresh
-- developer database has the same customer/account contract as production.
-- ============================================================================
...
create table if not exists public.recently_viewed (
  ...
  constraint recently_viewed_user_id_product_id_key unique (user_id, product_id)
);
...
create table if not exists public.brand_follows (
  ...
  created_at timestamptz not null default now(),
  constraint brand_follows_user_id_brand_slug_key unique (user_id, brand_slug)
);
create index if not exists brand_follows_brand_slug_idx on public.brand_follows (brand_slug);
...
create policy "Users can read their own follows"
  ...
  using (auth.uid() = user_id);
create policy "Users can add their own brand follow"
  on public.brand_follows for insert to authenticated
  with check (auth.uid() = user_id);
create policy "Users can remove their own brand follow"
...
grant all on table public.recently_viewed to anon, authenticated, service_role;
grant all on table public.brand_follows to anon, authenticated, service_role;
