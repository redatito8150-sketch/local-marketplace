...
begin
  for policy_row in
    ...
    from pg_policies p
    join targets t on t.table_name=p.tablename and t.policy_name=p.policyname
    where p.schemaname='public'
  loop
    using_clause := case when policy_row.qual is null then '' else ' using ('||replace(policy_row.qual,'auth.uid()','(select auth.uid())')||')' end;
    check_clause := case when policy_row.with_check is null then '' else ' with check ('||replace(policy_row.with_check,'auth.uid()','(select auth.uid())')||')' end;
    execute format('alter policy %I on public.%I%s%s',policy_row.policyname,policy_row.tablename,using_clause,check_clause);
  end loop;
end
$migration$;;
