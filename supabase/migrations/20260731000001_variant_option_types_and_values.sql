-- ============================================================================
-- INVENTORY & VARIANTS — option types and values.
--
-- Two system (global) option types are seeded here: Color and Size. A brand
-- may additionally create its own private option types (e.g. "Length") and
-- private values (e.g. a custom Size like "Petite") — both scoped by
-- brand_id, never visible/reusable by another brand. `brand_id is null`
-- means "system/global", exactly the same convention as everywhere else in
-- this schema uses a nullable owner column to mean "not privately owned".
--
-- Applied against an empty products/product_variants table (verified live:
-- 0 rows in both), so no data migration is needed here — this is a pure
-- additive schema build-out for the Inventory & Variants rebuild.
-- ============================================================================
...
-- System type names are globally unique; a brand's own private type names
-- are unique within that brand only (two different brands can each have
-- their own "Length" option).
create unique index if not exists option_types_system_key_key
  on option_types (key) where brand_id is null;
create unique index if not exists option_types_brand_key_key
  on option_types (brand_id, key) where brand_id is not null;

-- A brand can never create a private option type that shadows a reserved
-- system name ("color"/"size") — this must be a database-level guard, not
-- just app validation, since it's what prevents a "deceptive duplicate".
create or replace function public.enforce_option_type_not_reserved()
returns trigger
language plpgsql
as $$
begin
  if new.brand_id is not null and new.key in ('color', 'size') then
...
drop trigger if exists trigger_enforce_option_type_not_reserved on option_types;
...
create table if not exists option_values (
  ...
  label text not null,
  key text not null,
  -- Short, stable, uppercase code used to build a variant SKU
  -- deterministically (see 20260731000003) — never recomputed once a
  -- variant has actually used it, so a later label edit can't mutate an
  -- existing SKU.
  sku_token text not null,
  sort_order int not null default 0,
  swatch_type text check (swatch_type in ('single', 'split', 'multicolor')),
  primary_color text,
  secondary_color text,
  created_at timestamptz not null default now(),
  constraint option_values_primary_color_format
    check (primary_color is null or primary_color ~ '^#[0-9A-Fa-f]{6}$'),
  constraint option_values_secondary_color_format
    check (secondary_color is null or secondary_color ~ '^#[0-9A-Fa-f]{6}$'),
  constraint option_values_swatch_requires_colors check (
    swatch_type is null
    or (swatch_type = 'single' and primary_color is not null)
    or (swatch_type = 'split' and primary_color is not null and secondary_color is not null)
    or (swatch_type = 'multicolor')
  )
);

create unique index if not exists option_values_system_key_key
  on option_values (option_type_id, key) where brand_id is null;
create unique index if not exists option_values_brand_key_key
  on option_values (option_type_id, brand_id, key) where brand_id is not null;
create index if not exists idx_option_values_option_type_id on option_values (option_type_id);

alter table option_types enable row level security;
alter table option_values enable row level security;

-- Public read (same convention as products/taxonomy_nodes/brands — nothing
-- here is sensitive business data, and the storefront needs to display
-- resolved option labels on published products). No public write policy on
-- either table — every write goes through a service-role API route that
-- explicitly verifies the caller owns the target brand_id before writing,
-- same pattern as every other admin/brand-portal write path in this app.
create policy "Public can read option types" on option_types for select using (true);
