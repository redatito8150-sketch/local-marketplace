-- Bug fix: convert_application_to_brand() correctly set brands.owner_user_id
-- (so /brand-portal access already worked for a newly-converted owner via
-- requireBrandOwner(), which reads that column directly) but never updated
-- profiles.role to 'brand_owner' — every other access-granting path
-- (set_user_access RPC) always updates both together. The visible symptom:
-- the new owner's account still showed "Customer" everywhere that reads
-- profiles.role (the account role badge, and the Brand Portal link in the
-- account sidebar, which is gated on role — so it just didn't appear even
-- though the dashboard itself was reachable by direct URL).
--
-- Also drops the redundant `insert into brand_staff` for the new owner —
-- per the same set_user_access convention, owner_user_id alone is what
-- makes someone the owner; brand_staff is only for assistant-level members.
-- Harmless functionally (requireBrandOwner() already de-duplicates an
-- owner who also has a brand_staff row for the same brand), but wrong data
-- to have on file — it would show the owner listed as their own "staff".
...
begin
  ...
  -- Fix: the applicant becomes the brand's owner, not merely a staff
  -- member — this is the actual bug fix (was entirely missing before).
...
-- Grants are unaffected by create-or-replace, but restated for clarity —
-- this function must stay service_role-only, same as every other
...
-- Backfill: any application already converted before this fix landed,
-- whose applicant account is still stuck on role='customer', gets
-- corrected here too — this is a data-repair, not just a forward fix.
