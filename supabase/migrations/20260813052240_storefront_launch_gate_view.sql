...
--
-- SECOND CORRECTIVE PASS â€” items 1 and 2:
--
...
-- ORIGINAL "Public can read published products" RLS policy â€” which never
-- checked fulfillment_mode/first_stocked_at at all â€” would still hand back
...
-- that first defined it already uses) adding the full gate â€” launch state
-- AND no open fulfillment transition â€” directly into its `using()` clause.
...
-- is_mahaly_partner don't flip until activation â€” checkout already
...
-- entirely) the same defense in depth â€” RLS alone would leave
...
-- below, so the two can never drift out of sync with each other â€” the
...
-- rows via RLS) â€” an ordinary customer session evaluating this as a plain
...
-- ever returning a single boolean â€” no transition row data is exposed to
