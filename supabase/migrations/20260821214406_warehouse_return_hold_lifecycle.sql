...
-- Returns that were already physically in transit before this migration
-- predate the explicit dispatch columns. Preserve their open lifecycle by
-- recording the document's already-known requested quantity as the legacy
-- dispatched quantity. This is metadata-only: the reservation already
-- reduced sellable stock, so no stock quantity or ledger row is rewritten.
