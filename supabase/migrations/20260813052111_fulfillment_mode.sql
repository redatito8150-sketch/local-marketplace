...
-- via a trigger â€” every existing reader of is_mahaly_partner keeps working
...
--
-- Historical orders already carry their own immutable fulfillment snapshot â€”
...
-- brand can therefore never retroactively change what an old order shows â€”
...
-- the real switch. A plain insert (new brand row) is never blocked â€” only
...
-- ============================================================================
-- brand_fulfillment_transitions â€” one row per attempted mode switch. Every
...
-- Exactly one non-terminal transition per brand at a time â€” the atomic
...
-- â€” set by request_warehouse_transfer (supabase/migrations/
...
-- reached Zakhnook â€” a plain revert would silently lose track of it."
...
-- ============================================================================
-- Blocker computation â€” shared by request/revalidate/activate so the three
-- entry points can never disagree about what's still open. Returns a jsonb
-- array of short blocker codes (never free text â€” the API layer maps codes
...
begin
  ...
  -- in-flight (created/pending/paid/reflecting â€” not yet fulfilled,
  ...
  -- fetchOpenTransitionBrandSlugs check prevents from starting fresh â€” this
...
-- ============================================================================
-- request_fulfillment_mode_transition â€” creates the transition row and
...
-- of `quantity` (no longer sellable â€” cutover has begun, "prevent new
...
-- THEN the operation_key is checked â€” and a match only counts as a safe
...
begin
  ...
  exception when unique_violation then
    -- Lost a genuine concurrent race on operation_key â€” re-check exactly
  ...
  if p_to_mode = 'zakhnook_fulfilled' then
    ...
    -- becomes "declared, awaiting inbound transfer" â€” deterministic variant
    ...
    -- (v_lock.quantity) BEFORE the UPDATE zeroes it â€” the ledger row must
    ...
    loop
      ...
      select pv.id, pv.product_id, p_brand_id, v_original_quantity, -v_original_quantity, 0,
        'fulfillment_transition_snapshot', 'Fulfillment mode transition â€” cutover snapshot',
...
-- is received, an order finishes, etc. â€” or just polled) and advances its
-- status accordingly. Can also move a cleared transition BACK to a blocked
-- state if e.g. new stock reappeared â€” blockers are always recomputed from
...
-- reality changed in between) and â€” for zakhnook_fulfilled -> brand_fulfilled
-- â€” moves any residual brand_stock_quantity (credited back by a received
  ...
  if v_transition.to_mode = 'brand_fulfilled' then
    ...
    loop
      ...
      select pv.id, pv.product_id, v_transition.brand_id, pv.quantity, pv.brand_stock_quantity,
        ...
        'Fulfillment mode transition â€” activation', p_actor_id, 'fulfillment_transition',
...
--   the correct undo â€” but only for the portion that hasn't already moved:
...
--   instead atomically auto-cancelled here â€” otherwise it would be left
...
--   This direction's request step never snapshots anything â€” the brand
...
--   related_fulfillment_transition_id â€” see request_warehouse_return's own
...
begin
  ...
  -- received/partially_received means real stock has already moved â€”
  ...
  -- goods physically moving right now â€” not safely auto-cancellable, and
  ...
  -- shipped-yet state â€” for either direction, cancel_warehouse_document
  ...
  if v_transition.to_mode = 'zakhnook_fulfilled' then
    -- Undo the brand_fulfilled -> zakhnook_fulfilled cutover snapshot â€”
    ...
    loop
      ...
      select pv.id, pv.product_id, v_transition.brand_id, pv.quantity, pv.brand_stock_quantity,
        pv.quantity + pv.brand_stock_quantity, 'fulfillment_transition_snapshot',
        'Fulfillment mode transition cancelled â€” stock reverted',
        nullif(pg_catalog.btrim(p_notes), ''), p_actor_id, 'fulfillment_transition',
  ...
  -- to_mode = 'brand_fulfilled': deliberately no stock movement at all â€”
