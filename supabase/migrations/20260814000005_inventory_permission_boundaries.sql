...
--     sales of that brand's stock for the whole transition window — the
...
-- 20260812000006_master_orders.sql, already applied elsewhere — this repo's
...
begin
  ...
  -- Locked FIRST — establishes the brand-then-variants lock order every
...
begin
  ...
  -- Only genuinely UNRESOLVED quarantine blocks archiving — a line whose
...
begin
  ...
  loop
    ...
    -- status (not just 'pending'), counting only unreconciled lines — same
...
-- brand's variant regardless of mode — a brand_fulfilled brand's stock
...
-- reconciling this brand's balances during that window). Same signature —
