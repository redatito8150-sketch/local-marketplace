...
export default function NewArrivalsSection({ title, products, viewAllHref }: { title: string; products: Product[]; viewAllHref: string }) {
  ...
  return (
    <section className="mx-auto max-w-[1920px] border-b border-white/20 bg-cream/58 px-6 py-9 backdrop-blur-[2px] md:px-10 xl:px-16">
      <motion.div initial={{ opacity: 0, y: reduceMotion ? 0 : 18 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, amount: 0.5 }} transition={{ duration: 0.55 }} className="mb-5 flex items-center justify-between">
      ...
      <div ref={track} className="no-scrollbar flex snap-x snap-mandatory gap-5 overflow-x-auto pb-5 pt-1">
        {products.map((product, index) => (
          <motion.div
            key={product.id}
            initial={{ opacity: 0, x: reduceMotion ? 0 : 70, scale: reduceMotion ? 1 : 0.94 }}
            whileInView={{ opacity: 1, x: 0, scale: 1 }}
            viewport={{ once: true, amount: 0.22 }}
            transition={{ type: "spring", stiffness: 100, damping: 18, delay: reduceMotion ? 0 : Math.min(index, 5) * 0.08 }}
