...
import { AlertTriangle, CheckCircle2, Clock3, ExternalLink, Loader2, PackageCheck, Search, Truck, XCircle } from "lucide-react";
...
function TransferList({ transfers, emptyTitle, emptyDescription, brandParam }: { transfers: WarehouseTransferRow[]; emptyTitle: string; emptyDescription: string; brandParam?: string }) {
  ...
  return <><div className="divide-y divide-[#eee7e1]">{visible.map((transfer) => {
    ...
    return <article key={transfer.id} className="px-4 py-4 sm:px-5">
      ...
      <Link href={withBrand(`/brand-portal/warehouse/${transfer.id}`, brandParam)} className="mt-3 inline-flex h-9 items-center gap-1.5 rounded-xl bg-[#242424] px-3 text-[9.5px] font-bold text-white transition hover:bg-[#3a332e]">Open document<ExternalLink className="h-3 w-3" /></Link>
