...
export function ReturnRequestDrawer({ open, onClose, onSubmitted, variants, brandParam }: { open: boolean; onClose: () => void; onSubmitted: () => void; variants: WarehouseVariantRow[]; brandParam?: string }) {
  ...
  if (!open) return null;
  return <div ref={dialogRef} tabIndex={-1} className="fixed inset-0 z-50 flex justify-end outline-none" role="dialog" aria-modal="true" aria-labelledby={confirming ? "return-confirmation-title" : "return-request-title"}>
    ...
    {confirming ? <aside className="absolute inset-y-0 right-0 z-20 flex h-full w-full max-w-md flex-col overscroll-contain bg-[#fffdfb] shadow-[-20px_0_60px_rgba(61,43,31,.2)] lg:relative lg:right-auto">
    ...
    <aside className="relative flex h-full w-full max-w-2xl flex-col overscroll-contain bg-[#f6f2ed] shadow-[-20px_0_60px_rgba(61,43,31,.16)]">
      ...
      <div className="flex items-center gap-2 border-b border-[#ded5cd] px-5 py-3 sm:px-6"><BulkSelectionCheckbox checked={allResultsSelected} indeterminate={someResultsSelected && !allResultsSelected} label={allResultsSelected ? "Clear all products in search results" : "Select all products in search results"} onChange={(checked) => toggleVariants(selectableVariantIds, checked)} className={!selectableVariantIds.length ? "pointer-events-none opacity-40" : ""} /><label className="relative min-w-0 flex-1"><span className="sr-only">Search return inventory</span><Search aria-hidden="true" className="pointer-events-none absolute left-3.5 top-1/2 h-3.5 w-3.5 -translate-y-1/2 text-[#9d9086]" /><input value={query} onChange={(event) => { setQuery(event.target.value); setPage(1); }} autoComplete="off" placeholder="Search product, color, size or SKU…" className="h-10 w-full rounded-xl bg-white pl-9 pr-3 text-[11px] text-[#403730] outline-none ring-1 ring-[#e6ddd6] transition focus-visible:ring-2 focus-visible:ring-[#C85956]/30" /></label></div>
      <div className="min-h-0 flex-1 overflow-y-auto px-5 py-4 sm:px-6">
        {error ? <p role="alert" className="mb-3 rounded-xl bg-red-50 px-4 py-3 text-[10.5px] font-semibold text-red-800">{error}</p> : null}
...
export default function WarehouseExperience({ transfers, brandParam, readOnly = false }: { transfers: WarehouseTransferRow[]; brandParam?: string; readOnly?: boolean }) {
  ...
  const filteredTransfers = useMemo(() => transfers
    .filter((transfer) => documentFilter === "all" || (documentFilter === "requested" && ["pending", "submitted"].includes(transfer.status)) || (documentFilter === "preparing" && transfer.status === "approved") || (documentFilter === "in_transit" && ["in_transit", "partially_received"].includes(transfer.status)) || (documentFilter === "received" && transfer.status === "received") || (documentFilter === "action_required" && hasOpenDocumentIssue(transfer)))
  ...
  const statusCounts: Record<DocumentFilter, number> = {
    ...
    in_transit: transfers.filter((transfer) => ["in_transit", "partially_received"].includes(transfer.status)).length,
