...
import { Check, ChevronLeft, ChevronRight, Clock3, Download, Filter, PackageCheck, Printer, RotateCcw, Search, Truck, X } from "lucide-react";
...
function productsSubtotal(order: BrandOrder) {
  return order.items.reduce((sum, item) => sum + (item.currency === "EGP" ? item.price * item.quantity : 0), 0);
}

function orderTotal(order: BrandOrder) {
  return productsSubtotal(order) + order.shippingFeeEgp;
}
      ...
      {selected && <div className="order-print-overlay fixed inset-0 z-[90] flex justify-end bg-[#241c18]/25 backdrop-blur-[1px]" onMouseDown={(event) => event.target === event.currentTarget && setSelected(null)}>
        <aside data-order-drawer role="dialog" aria-modal="true" aria-label={`Order ${selected.orderNumber}`} className="h-full w-full overflow-y-auto bg-[#fffdfb] shadow-[-24px_0_70px_rgba(36,28,24,.16)] sm:max-w-[520px]">
          <div data-order-print className="hidden">
            ...
            <div className="mt-5 grid grid-cols-3 gap-3"><div className="rounded-xl border border-[#ddd3cb] p-3"><p className="text-[9px] font-bold uppercase tracking-[0.12em] text-[#8d8076]">Customer</p><p className="mt-2 text-[11px] font-bold text-[#242424]">{selected.shippingName}</p><p className="mt-1 text-[10px] text-[#6f635b]">{selected.shippingCity}, {selected.shippingGovernorate}</p></div><div className="rounded-xl border border-[#ddd3cb] p-3"><p className="text-[9px] font-bold uppercase tracking-[0.12em] text-[#8d8076]">Payment</p><p className="mt-2 text-[11px] font-bold text-[#242424]">{label(selected.paymentMethod)}</p><p className="mt-1 text-[10px] text-[#6f635b]">{label(selected.paymentStatus)}</p></div><div className="rounded-xl border border-[#ddd3cb] p-3"><p className="text-[9px] font-bold uppercase tracking-[0.12em] text-[#8d8076]">Fulfillment</p><p className="mt-2 text-[11px] font-bold text-[#242424]">{selected.fulfillmentType === "mahaly_pool" ? "Zakhnook" : "Brand direct"}</p><p className="mt-1 text-[10px] text-[#6f635b]">{selected.fulfillmentType === "mahaly_pool" ? "Marketplace managed" : "Brand handoff"}</p></div></div>
            <section className="mt-6"><h2 className="text-[10px] font-extrabold uppercase tracking-[0.14em] text-[#6f635b]">Products</h2><table className="mt-2 w-full border-collapse text-left"><thead><tr className="border-y border-[#d9cfc7] text-[9px] uppercase tracking-[0.08em] text-[#8d8076]"><th className="py-2 font-bold">Product</th><th className="py-2 font-bold">Color / Size</th><th className="py-2 text-center font-bold">Qty</th><th className="py-2 text-right font-bold">Total</th></tr></thead><tbody>{selected.items.map((item) => <tr key={item.id} className="border-b border-[#ebe4de] text-[10.5px] text-[#3f3731]"><td className="py-2.5 font-bold">{item.name}</td><td className="py-2.5">{item.color || "No color"} / {formatSize(item.size)}</td><td className="py-2.5 text-center">{item.quantity}</td><td className="py-2.5 text-right font-bold">{hasItemDiscount(item) && <span className="mr-1.5 font-normal text-[#8d8076] line-through">{formatPrice(item.originalPrice! * item.quantity, item.currency)}</span>}{formatPrice(item.price * item.quantity, item.currency)}</td></tr>)}</tbody></table><div className="ml-auto mt-3 w-56 space-y-1.5 text-[10px]"><div className="flex justify-between"><span className="text-[#756960]">Products subtotal</span><span className="font-bold">{formatPrice(productsSubtotal(selected), "EGP")}</span></div><div className="flex justify-between"><span className="text-[#756960]">Delivery</span><span className="font-bold">{selected.shippingFeeEgp > 0 ? formatPrice(selected.shippingFeeEgp, "EGP") : "Free"}</span></div><div className="flex justify-between border-t border-[#d9cfc7] pt-1.5 text-[11px]"><span className="font-extrabold">Order total</span><span className="font-extrabold">{formatPrice(orderTotal(selected), "EGP")}</span></div></div></section>
          ...
          <div className="space-y-5 p-5">
            <section className="overflow-hidden rounded-2xl border border-[#eadfd7] bg-white">
              ...
              <div className="divide-y divide-[#f0e9e3] px-4">{selected.items.map((item) => <div key={item.id} className="flex gap-3 py-3.5"><OrderImage item={item} size="lg" /><div className="min-w-0 flex-1"><div className="flex items-start justify-between gap-3"><div className="min-w-0"><p className="truncate text-[12px] font-bold text-[#403730]">{item.name}</p><p className="mt-1 text-[10.5px] text-[#887a70]">{item.color || "No color"} · {formatSize(item.size)}</p></div><div className="flex-none text-right">{hasItemDiscount(item) && <p className="text-[9.5px] text-[#a09287] line-through">{formatPrice(item.originalPrice! * item.quantity, item.currency)}</p>}<p className={`text-[11px] font-extrabold ${hasItemDiscount(item) ? "text-[#C85956]" : "text-[#403730]"}`}>{formatPrice(item.price * item.quantity, item.currency)}</p></div></div><p className="mt-2 text-[10.5px] text-[#887a70]">Quantity {item.quantity}</p></div></div>)}</div>
              <div className="border-t border-[#eee7e1] bg-[#fcfaf8] px-4 py-4"><div className="space-y-2 text-[11px]"><div className="flex items-center justify-between text-[#74675e]"><span>Products subtotal</span><span className="font-semibold text-[#4a4039]">{formatPrice(productsSubtotal(selected), "EGP")}</span></div><div className="flex items-center justify-between text-[#74675e]"><span>Delivery</span><span className="font-semibold text-[#4a4039]">{selected.shippingFeeEgp > 0 ? formatPrice(selected.shippingFeeEgp, "EGP") : "Free"}</span></div></div><div className="mt-3 flex items-end justify-between border-t border-[#e8dfd8] pt-3"><div><p className="text-[10px] font-bold uppercase tracking-[0.1em] text-[#9b8d82]">Order total</p><p className="mt-1 text-[10px] text-[#81746b]">{label(selected.paymentMethod)} · {label(selected.paymentStatus)}</p></div><p className="text-lg font-extrabold tracking-[-0.03em] text-[#242424]">{formatPrice(orderTotal(selected), "EGP")}</p></div></div>
            </section>
