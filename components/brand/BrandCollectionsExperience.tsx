    ...
    <>
      ...
      <div className="mt-3 flex items-start justify-between gap-3">
        <div className="min-w-0">
          <h4 className="truncate text-[13px] font-semibold text-[#242424]">{product.name}</h4>
...
export default function BrandCollectionsExperience({
  ...
  const supporting = collections.slice(1);
    ...
    <div className="space-y-4">
      <div className="flex items-end justify-between gap-6 pb-3">
        <div>
          ...
          <InlineEditableText
            ...
            as="h1"
            className="mt-2 font-serif text-3xl tracking-[-.02em] text-[#2e2722] sm:text-4xl"
          />
        </div>
      ...
      {!featured && (
        <div className="flex flex-col items-center justify-center rounded-[22px] border border-dashed border-[#d8cabc] bg-[#fffaf5] px-6 py-16 text-center">
          ...
          <h2 className="mt-4 text-lg font-bold text-[#242424]">No collections yet</h2>
      ...
      {featured && activeId !== featured.id ? (
        ...
        >
          <CollectionCoverCarousel
            images={featured.coverImages}
            alt=""
            priority
            sizes="(max-width: 1400px) 100vw, 1280px"
            fillClassName="absolute inset-0 transition duration-1000 ease-out group-hover:scale-[1.025]"
          />
          <div className="pointer-events-none absolute inset-0 bg-[linear-gradient(90deg,rgba(25,24,21,.62)_0%,rgba(25,24,21,.2)_45%,rgba(25,24,21,.02)_72%)]" />
          <div className="pointer-events-none absolute inset-y-0 left-0 flex max-w-xl flex-col justify-center p-7 sm:p-11 lg:p-14">
        ...
        <section key={`featured-${featured.id}`} className="collection-feature-enter overflow-hidden rounded-[22px] border border-[#e6d9ce] bg-[#fffaf5] shadow-[0_16px_50px_rgba(63,42,31,.07)]">
          <div className="grid lg:grid-cols-[0.72fr_1.28fr]">
            <div className="relative min-h-[330px] overflow-hidden lg:min-h-[560px]">
              <CollectionCoverCarousel images={featured.coverImages} alt="" sizes="(max-width: 1024px) 100vw, 38vw" fillClassName="absolute inset-0" />
              <div className="pointer-events-none absolute inset-0 bg-gradient-to-t from-black/75 via-black/15 to-transparent" />
              <button type="button" onClick={() => setActiveId(null)} aria-label="Close collection" className="absolute right-4 top-4 z-10 grid h-10 w-10 place-items-center rounded-full bg-white/90 text-[#3d302b] backdrop-blur"><X className="h-4 w-4" /></button>
              <div className="pointer-events-none absolute inset-x-0 bottom-0 p-7 sm:p-9"><CollectionCopy item={featured} compact /></div>
            </div>
            <div className="flex min-w-0 flex-col justify-center p-5 sm:p-8 lg:p-9">
              <div className="mb-6 flex items-center justify-between gap-4">
                <div>
                  ...
                  <h3 className="mt-1 font-serif text-2xl text-[#242424]">
            ...
            return (
              ...
              >
                <CollectionCoverCarousel
                  ...
                  fillClassName={`absolute inset-0 transition duration-700 ${isCompressed ? "group-hover:scale-105" : "group-hover:scale-[1.035]"}`}
                />
