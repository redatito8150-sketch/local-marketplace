    ...
    <header className="bg-[#f5eee5] pt-3">
      <div className="mx-auto max-w-brand px-4 sm:px-6 lg:px-10">
        <div className="relative min-h-[280px] overflow-hidden rounded-[28px] sm:min-h-[350px]">
          <Image src={brand.heroImage} alt={`${brand.name} cover`} fill priority sizes="(max-width: 1440px) 100vw, 1360px" className="object-cover" />
          ...
          <div className="relative flex min-h-[280px] items-end p-5 sm:min-h-[350px] sm:p-8 lg:p-10">
            <div className="flex w-full flex-col gap-5 sm:flex-row sm:items-end sm:justify-between">
              <div className="flex items-end gap-4">
                <div className="relative flex h-20 w-20 shrink-0 items-center justify-center overflow-hidden rounded-2xl border-4 border-white bg-[#fffaf4] font-serif text-xl text-[#7f1d2d] shadow-xl sm:h-24 sm:w-24">{brand.logoImage ? <Image src={brand.logoImage} alt={`${brand.name} logo`} fill sizes="96px" className="object-cover" /> : initials(brand.name)}</div>
                <div className="pb-1 text-white">
                  <div className="flex items-center gap-2"><h1 className="font-serif text-3xl font-semibold tracking-tight sm:text-4xl">{brand.name}</h1><BadgeCheck className="h-5 w-5 text-[#e9c477]" aria-label="Verified brand" /></div>
                  <p className="mt-1 flex flex-wrap items-center gap-x-3 gap-y-1 text-[12px] text-white/80"><span>{brand.category}</span><span className="inline-flex items-center gap-1"><MapPin className="h-3.5 w-3.5" />{brand.city}</span>{brand.foundedYear && <span>Since {brand.foundedYear}</span>}</p>
                </div>
              </div>
