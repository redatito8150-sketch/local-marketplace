  ...
  return (
    <section className="overflow-hidden rounded-[22px] bg-[#ece7e0] shadow-[0_12px_32px_rgba(72,50,36,.07)]">
            ...
            <article key={item.id} className={`px-5 py-4 ${editingItemId === item.id ? "bg-[#f2ece6]" : ""}`}>
              <div className="flex flex-col gap-3 xl:flex-row xl:items-center">
                ...
                <div className="flex flex-wrap items-end gap-2 xl:justify-end">
                  ...
                  <button type="button" onClick={() => editingItemId === item.id ? setEditingItemId(null) : startIssue(item)} className="inline-flex h-10 items-center gap-1.5 rounded-xl bg-[#f8f4f0] px-3 text-[9.5px] font-bold text-[#62564d] transition hover:bg-[#242424] hover:text-white"><CircleAlert className="h-3.5 w-3.5" />Report issue<ChevronDown className={`h-3 w-3 transition-transform ${editingItemId === item.id ? "rotate-180" : ""}`} /></button>
                  <Link href={`/admin/inventory?view=activity&brand=${encodeURIComponent(brandSlug)}&variantId=${encodeURIComponent(currentVariantId)}`} className="inline-flex h-10 items-center gap-1 rounded-xl bg-[#e2dcd4] px-3 text-[9.5px] font-bold text-[#5b5049] hover:bg-[#242424] hover:text-white"><Activity className="h-3 w-3" />Ledger</Link>
                </div>
              </div>
              ...
              {editingItemId === item.id ? (
                <div className="mt-4 rounded-2xl bg-[#f8f4f0] p-4">
                  ...
                  <div className="mt-2 grid gap-2 sm:grid-cols-2 xl:grid-cols-5">{(Object.entries(ISSUE_META) as Array<[IssueKind, { label: string; description: string }]>).map(([value, meta]) => <button key={value} type="button" onClick={() => { setKind(value); setError(""); }} className={`rounded-xl px-3 py-2.5 text-left transition ${kind === value ? "bg-[#242424] text-white" : "bg-white text-[#62564d] hover:bg-[#eee7e1]"}`}><span className="block text-[10px] font-extrabold">{meta.label}</span><span className={`mt-1 block text-[8.5px] leading-4 ${kind === value ? "text-white/65" : "text-[#8d8076]"}`}>{meta.description}</span></button>)}</div>
      ...
      <div className="border-t border-[#ddd4cc] px-5 py-4">
        <div className="flex flex-wrap items-center justify-between gap-3"><div><p className="text-[10px] font-bold text-[#62564d]">{formatCount(drafts.length)} issue{drafts.length === 1 ? "" : "s"} ready for review</p><p className="mt-1 text-[9px] text-[#8d8076]">Original document: unchanged. A different administrator must approve it.</p></div><button type="button" onClick={() => setReviewing(true)} disabled={!drafts.length} className="inline-flex h-11 items-center rounded-xl bg-[#242424] px-5 text-[11px] font-bold text-white hover:bg-[#3a332e] disabled:opacity-40"><ShieldCheck className="mr-2 h-4 w-4" />Review corrections · {formatCount(drafts.length)}</button></div>
        {reviewing ? <div className="mt-4 rounded-2xl bg-[#f8f4f0] p-4"><div className="flex items-start gap-3"><span className="flex h-9 w-9 items-center justify-center rounded-xl bg-amber-50 text-amber-800"><PackageCheck className="h-4 w-4" /></span><div><h3 className="text-[13px] font-extrabold text-[#302924]">Correction review</h3><p className="mt-1 text-[10px] leading-5 text-[#756960]">These changes create one linked correction document. Nothing posts to inventory until a different administrator approves it.</p></div></div><div className="mt-3 space-y-2">{drafts.map((draft) => <div key={draft.id} className="flex flex-col gap-2 rounded-xl bg-white px-3 py-2.5 sm:flex-row sm:items-center"><div className="min-w-0 flex-1"><p className="text-[10.5px] font-extrabold text-[#403730]">{draft.label}</p><p className="mt-1 text-[9.5px] text-[#756960]">{draft.preview}</p><p className="mt-1 text-[9px] text-[#94867c]">{draft.note}</p></div><button type="button" onClick={() => setDrafts((current) => current.filter((candidate) => candidate.id !== draft.id))} className="inline-flex h-8 items-center gap-1 px-2 text-[9px] font-bold text-red-700"><Trash2 className="h-3.5 w-3.5" />Remove</button></div>)}</div><div className="mt-3 flex flex-wrap gap-2"><button type="button" onClick={requestCorrection} disabled={busy || !drafts.length} className="inline-flex h-11 items-center rounded-xl bg-[#C85956] px-5 text-[11px] font-bold text-white hover:bg-[#b84e4b] disabled:opacity-45">{busy ? <Loader2 className="mr-2 h-4 w-4 animate-spin" /> : <CheckCircle2 className="mr-2 h-4 w-4" />}{busy ? "Creating…" : "Create correction for approval"}</button><button type="button" onClick={() => setReviewing(false)} disabled={busy} className="h-11 px-3 text-[10px] font-bold text-[#62564d]">Back to document</button></div></div> : null}
      </div>
      ...
      {corrections.length ? <details className="group border-t border-[#ddd4cc] px-5 py-4"><summary className="flex cursor-pointer list-none items-center gap-2 text-[11px] font-extrabold text-[#403730] outline-none [&::-webkit-details-marker]:hidden"><FilePenLine className="h-3.5 w-3.5 text-[#C85956]" />Correction documents · {formatCount(corrections.length)}<ChevronDown className="ml-auto h-3.5 w-3.5 transition-transform group-open:rotate-180" /></summary><div className="mt-3 space-y-2">{corrections.map((correction) => {
        ...
        return <article key={correction.id} className="rounded-xl bg-[#f8f4f0] p-3"><div className="flex flex-wrap items-center gap-2"><p className="text-[10.5px] font-extrabold text-[#302924]">{correction.correctionNumber}</p><span className={`rounded-lg px-2 py-1 text-[8.5px] font-bold ${correctionTone(correction.status)}`}>{titleCase(correction.status)}</span><span className="text-[9px] text-[#8d8076]">{titleCase(correction.correctionType)}</span>{correction.status === "pending_approval" ? <div className="flex gap-2 sm:ml-auto"><button type="button" onClick={() => { setRejecting(rejecting === correction.id ? null : correction.id); setReversing(null); }} className="inline-flex h-8 items-center rounded-lg bg-white px-2.5 text-[9px] font-bold text-[#675b52]"><XCircle className="mr-1 h-3 w-3" />Reject</button><button type="button" onClick={() => approve(correction.id)} disabled={Boolean(approving) || busy} className="inline-flex h-8 items-center rounded-lg bg-[#242424] px-2.5 text-[9px] font-bold text-white disabled:opacity-45">{approving === correction.id ? <Loader2 className="mr-1 h-3 w-3 animate-spin" /> : <ShieldCheck className="mr-1 h-3 w-3" />}Approve & post</button></div> : canReverse ? <button type="button" onClick={() => { setReversing(reversing === correction.id ? null : correction.id); setRejecting(null); }} className="inline-flex h-8 items-center rounded-lg bg-white px-2.5 text-[9px] font-bold text-[#675b52] sm:ml-auto"><RotateCcw className="mr-1 h-3 w-3" />Request reversal</button> : null}</div><p className="mt-2 text-[9.5px] leading-4 text-[#62564d]">{correction.note}</p><div className="mt-2 flex flex-wrap gap-1.5">{correction.lines.map((line) => <span key={line.id} className="rounded-lg bg-[#eee7e1] px-2 py-1 text-[8.5px] font-semibold text-[#62564d]">{titleCase(line.action)} · {formatCount(line.quantity)}</span>)}</div>{correction.rejectionNote ? <p className="mt-2 rounded-lg bg-red-50 px-2.5 py-2 text-[9px] font-semibold text-red-800">Rejected: {correction.rejectionNote}</p> : null}{rejecting === correction.id ? <div className="mt-3 flex flex-col gap-2 sm:flex-row"><input value={rejectionNote} onChange={(event) => setRejectionNote(event.target.value)} placeholder="Required rejection reason" className={`${CONTROL} flex-1 bg-white`} /><button type="button" onClick={() => reject(correction.id)} disabled={rejectionNote.trim().length < 5 || busy} className="h-10 rounded-xl bg-red-700 px-3 text-[9.5px] font-bold text-white disabled:opacity-45">Confirm rejection</button></div> : null}{reversing === correction.id ? <div className="mt-3"><p className="mb-2 text-[9px] text-[#8d8076]">A reversal does not erase this document; it creates a separately approved counter-entry.</p><div className="flex flex-col gap-2 sm:flex-row"><input value={reversalNote} onChange={(event) => setReversalNote(event.target.value)} placeholder="Why must this posted correction be reversed?" className={`${CONTROL} flex-1 bg-white`} /><button type="button" onClick={() => requestReversal(correction.id)} disabled={reversalNote.trim().length < 5 || busy} className="h-10 rounded-xl bg-[#242424] px-3 text-[9.5px] font-bold text-white disabled:opacity-45">Create reversal</button></div></div> : null}</article>;
      })}</div></details> : null}
    </section>
  );
}
