  ...
  return (
    <section className="overflow-hidden rounded-[22px] bg-[#ece7e0] shadow-[0_12px_32px_rgba(72,50,36,.07)]">
      ...
      {!readOnly ? <div className="border-t border-[#ddd4cc] px-5 py-4">
        <div className="flex flex-wrap items-center justify-between gap-3"><div><p className="text-[10px] font-bold text-[#62564d]">{formatCount(drafts.length)} issue{drafts.length === 1 ? "" : "s"} ready for review</p><p className="mt-1 text-[9px] text-[#8d8076]">Original document: unchanged. Full Admin corrections apply immediately; delegated warehouse staff require independent approval.</p></div><button type="button" onClick={() => setReviewing(true)} disabled={!drafts.length} className="inline-flex h-11 items-center rounded-xl bg-[#242424] px-5 text-[11px] font-bold text-white hover:bg-[#3a332e] disabled:opacity-40"><ShieldCheck className="mr-2 h-4 w-4" />Review corrections · {formatCount(drafts.length)}</button></div>
