...
export default function ReviewForm({item}:{item:{id:string;product_id:string|null;name:string;image:string}}){
 ...
 return <form onSubmit={submit} className="rounded-[24px] border border-[#e4d8ce] bg-white p-5 shadow-sm">
  ...
  <div className="flex items-center gap-3"><span className="relative h-14 w-14 overflow-hidden rounded-xl bg-[#f3ece5]"><Image src={item.image} alt="" fill sizes="56px" className="object-cover"/></span><div><p className="text-[10px] font-bold uppercase tracking-[.14em] text-[#C85956]">Verified purchase</p><h3 className="font-semibold text-[#332a26]">{item.name}</h3></div></div>
  <fieldset className="mt-5"><legend className="text-sm font-semibold">Your rating</legend><div className="mt-2 flex items-center gap-1">{[1,2,3,4,5].map(star=><button type="button" key={star} onClick={()=>setRating(star)} aria-label={`${star} stars — ${LABELS[star]}`} className="rounded p-1 focus:outline-none focus:ring-2 focus:ring-[#C85956]"><Star className={`h-7 w-7 ${star<=rating?"fill-[#b47b24] text-[#b47b24]":"text-[#d8cec5]"}`}/></button>)}<span aria-live="polite" className="ml-2 text-xs text-[#766a62]">{LABELS[rating]}</span></div></fieldset>
  ...
  {message&&<p aria-live="polite" className="mt-4 text-sm text-[#C85956]">{message}</p>}
  <button disabled={busy||!rating} className="mt-5 min-h-11 rounded-full bg-[#C85956] px-6 text-sm font-bold text-white disabled:opacity-50">{busy?"Publishing…":"Publish review"}</button>
 </form>
}
