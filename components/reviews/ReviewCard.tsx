      ...
      <article className="group rounded-[18px] border border-[#e8ddd3] bg-[#fffdfa] px-5 py-6 shadow-[0_7px_25px_rgba(67,45,34,.028)] sm:px-7 sm:py-7">
        <div className="grid gap-6 sm:grid-cols-[180px_1fr] lg:grid-cols-[200px_1fr]">
          <div className="flex items-start gap-3 sm:block">
            <div className="grid h-14 w-14 shrink-0 place-items-center overflow-hidden rounded-full bg-[#8f2634] font-serif text-xl text-white">
          ...
          <div className="min-w-0">
            ...
            {review.title && <h3 className="mt-4 text-[17px] font-semibold text-[#302824]">{review.title}</h3>}
            ...
            {review.reply && (
              <section className="mt-5 rounded-xl border-l-[3px] border-[#8f2335] bg-[#fff7f5] p-4" aria-label={`Response from ${review.reply.brandName}`}>
                <h4 className="flex items-center gap-2 text-xs font-bold text-[#8f2335]"><MessageSquareQuote className="h-4 w-4" />Response from {review.reply.brandName}</h4>
  ...
  return (
    <article className="rounded-[24px] border border-[#e9dfd6] bg-white p-5 shadow-[0_10px_35px_rgba(67,45,34,.055)] sm:p-7">
      <div className="flex flex-wrap items-start justify-between gap-4">
        <div><p className="font-semibold text-[#302824]">{review.authorName}</p><div className="mt-1.5 flex items-center gap-2"><RatingStars rating={review.rating} /><span className="inline-flex items-center gap-1 text-[11px] font-semibold text-[#68765f]"><BadgeCheck className="h-3.5 w-3.5" />Verified Purchase</span></div></div>
