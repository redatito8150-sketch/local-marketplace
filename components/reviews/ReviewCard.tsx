...
export default function ReviewCard({ review }: { review: PublicReview }) {
  ...
  return <article className="rounded-[24px] border border-[#e9dfd6] bg-white p-5 shadow-[0_10px_35px_rgba(67,45,34,.055)] sm:p-7">
    <div className="flex flex-wrap items-start justify-between gap-4">
      ...
      <time className="text-xs text-[#8b8078]">{new Date(review.createdAt).toLocaleDateString("en-US",{year:"numeric",month:"short",day:"numeric"})}{edited?" · Edited":""}</time>
    </div>
    ...
    {review.reply&&<section className="mt-5 rounded-2xl border-l-4 border-[#8f2335] bg-[#fff7f5] p-4" aria-label={`Response from ${review.reply.brandName}`}><h4 className="flex items-center gap-2 text-xs font-bold text-[#8f2335]"><MessageSquareQuote className="h-4 w-4"/>Response from {review.reply.brandName}</h4><p className="mt-2 text-sm leading-6 text-[#665a54]">{review.reply.body}</p><time className="mt-2 block text-[11px] text-[#9a8c84]">{new Date(review.reply.createdAt).toLocaleDateString("en-US")}{review.reply.updatedAt!==review.reply.createdAt?" · Edited":""}</time></section>}
