...
export function brandProductToProduct(item: Brand["products"][number]): Product {
  return { id: item.id, name: item.name, brand_name: item.brand, brand_slug: null, category: null, product_category: null, price: item.price, compare_at_price: item.compareAtPrice ?? null, currency: item.currency, image: item.image, images: [item.image], rating: 0, review_count: 0, colors: [], sizes: [], unavailable_sizes: [], description: "", details: [], care_instructions: [], shipping_returns: "", in_stock: item.inStock, track_inventory: false };
}
