...
export default function OrdersRoute() {
  const router = useRouter(); const { spacing } = useAppTheme();
  ...
  return <AuthGuard><Stack.Screen options={{ headerShown: true, title: "Orders" }} />{query.isLoading ? <LoadingState /> : query.isError ? <ErrorState message="We couldn't load your orders." onRetry={() => query.refetch()} /> :
    <FlatList data={query.data ?? []} keyExtractor={(item) => item.id} refreshing={query.isRefetching} onRefresh={() => query.refetch()} contentContainerStyle={{ padding: spacing.md, gap: spacing.sm, flexGrow: 1 }}
      ...
      renderItem={({ item }) => <Pressable onPress={() => router.push(`/orders/${item.id}` as Href)}><Card><View style={{ gap: spacing.xs }}><View style={{ flexDirection: "row", justifyContent: "space-between" }}><AppText variant="label">#{item.orderNumber}</AppText><Badge tone={item.status === "fulfilled" ? "success" : item.status === "cancelled" ? "warning" : "neutral"}>{statusLabel[item.status]}</Badge></View><AppText variant="caption">{new Date(item.createdAt).toLocaleDateString("en-EG")} · {item.items.length} items</AppText>{item.subtotalEgp ? <AppText variant="label">{formatPrice(item.subtotalEgp - item.discountAmountEgp, "EGP")}</AppText> : null}</View></Card></Pressable>} />}
  </AuthGuard>;
}
