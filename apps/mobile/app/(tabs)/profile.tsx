...
export default function ProfileRoute() {
  ...
  return <AuthGuard>{profile.isLoading ? <LoadingState /> : <Screen scroll><View style={{ gap: spacing.md, paddingVertical: spacing.lg }}><AppText variant="display">{profile.data?.fullName || "Your Mahaly"}</AppText><AppText style={{ color: colors.textMuted }}>{profile.data?.email}</AppText>
    ...
    <Card style={{ gap: spacing.sm }}>{links.map((item, index) => <View key={item[0]}>{index ? <Divider /> : null}<Pressable accessibilityRole="button" accessibilityLabel={item[0]} onPress={() => router.push(item[1] as Href)} style={{ flexDirection: "row", alignItems: "center", gap: spacing.sm, paddingVertical: spacing.sm }}><Ionicons name={item[2]} size={21} color={colors.primary} /><AppText variant="label" style={{ flex: 1 }}>{item[0]}</AppText><Ionicons name="chevron-forward" size={18} color={colors.textMuted} /></Pressable></View>)}</Card>
