...
import { getCatalogFacets, getProductPage, ProductSort } from "@/domain/products";
...
export default function ProductsRoute() {
  ...
  const query = useInfiniteQuery({
    queryKey: ["products", "listing", params.category, sort, brand, color, size, inStock, discounted, minimumRating],
    initialPageParam: 0,
  ...
  const activeCount = [brand, color, size, inStock, discounted, minimumRating].filter(Boolean).length;
  const resetFilters = () => { setBrand(""); setColor(""); setSize(""); setInStock(false); setDiscounted(false); setMinimumRating(0); };
  return (
    <>
